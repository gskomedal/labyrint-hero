...
- **LH2 iterasjon 3 (levende terreng, grunnstoffer som i LH1):**
  - **Direkte grunnstoffkilder kraftig nedskalert:** grunnstoffer utvinnes nå i hovedsak fra mineraler via smelting, som i LH1. Kun sjeldne unntak i dypet: maks én gasslomme (Ar fra Dyplag, He/Ne/Kr i Underverden, Xe/Rn i kjernen) og ett gedigent funn (Au/Ag i Underverden, Au/Pt/Ir i kjernen) per sone, med sannsynlighet under 100 %. Ingen på overflaten
  - **Dyreliv på overflaten:** hoppende kaniner (flykter fra spilleren) og kretsende fugler – ren atmosfære
...
- **LH2 iterasjon 11 (3D-karaktermodell + 3D karakterskaper + inventory-fiks):**
  - **Bugfiks – inventaret var tomt:** LH1s `InventoryScene` kalte `ItemGraphics.drawInventoryIcon`, men `ItemGraphics` var ikke lastet i LH2, så scenen krasjet ved første gjenstand og ryggsekken ble blank. `src/graphics/ItemGraphics.js` lastes nå – gjenstander, ikoner, stabeltall og navn vises korrekt
  - **Ny detaljert 3D-figur (`CharacterModel.js`):** delt mellom verden og skaperen. Avsmalnende lemmer (lavpoly-sylindere), ansikt (øyne med iris, bryn, nese, munn), 10 frisyrer, 4 skjeggtyper, antrekk som påvirker formen (tunika/kappe/vest/kåpe med slep og rygg-kappe), belte med spenne, støvler, og raseproporsjoner (brede dverger, slanke alver, små hobbiter). Hakke holdes i hånden ved graving
  - **3D karakterskaper (`ui/CharacterCreator3D.js`):** erstatter LH1s 2D-pikselforhåndsvisning med en **levende, roterende 3D-figur** på en dreieskive. HTML/CSS-kontroller for rase, kjønn, hud/hår/øyne/klesfarge, frisyre, skjegg, antrekk, navn og startbonus – alt oppdaterer figuren direkte. Åpnes automatisk ved første oppstart; valgene farger 3D-figuren i verden
