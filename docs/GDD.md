...
- **Labyrintelementer:** en åpen ruinlabyrint på overflaten med tier+1-mineraler i blindveiene – og under bakken er **hver grottesone en komplett hulelabyrint**: brede ganger (6 enheter), vegger fra gulv til tak og massivt steintak, så layouten aldri kan ses ovenfra. Tre åpne krystallkamre per sone. Nedgangsportalen ligger på flisen lengst unna spawn (BFS), så man må løse labyrinten for å komme dypere. Kameraet har kollisjon og klemmes under taket.
...
- **Gjenbrukt LH1-UI:** SkillScene (K), ElementBookScene (B), MineralWikiScene (V), SmelteryScene (smelteovnen, med leirlager/`campStash`), ChemLabScene (kjemibordet) og InventoryScene (Tab, med karakterportrett og utstyr) kjører uendret i en gjennomsiktig Phaser-instans oppå Three.js (`LH1UIHost`). Phaser er vendored i `lh2/vendor/`. Vitenskaps-XP gis via wrappere rundt `SmeltingSystem`/`ChemistrySystem` i `lh1-shims.js`.
